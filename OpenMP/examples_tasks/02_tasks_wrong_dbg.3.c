   ...
   #pragma omp single
    {
     ...
     #pragma omp task shared(result1)
      {
	PRINTF(" + Thread %d is executing T1 %p\n", omp_get_thread_num(), &result1);
	...
	PRINTF(" + Thread %d has got T1 = %g\n", omp_get_thread_num(), result1);
     ...
     #pragma omp task shared(result2)
      {
	PRINTF(" + Thread %d is executing T2 %p\n", omp_get_thread_num(), &result2);
       	for( int jj = 0; jj < N; jj++ )
	...
	PRINTF(" + Thread %d has got T2 = %g\n", omp_get_thread_num(), result2);
     ...
     #pragma omp task
      {
	int jj;
	PRINTF(" + Thread %d is executing T3 %p %p\n", omp_get_thread_num(), &result3, &jj);
       	for( jj = 0; jj < N; jj++ )
